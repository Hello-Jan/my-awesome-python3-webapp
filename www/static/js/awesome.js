...
if (! String.prototype.trim){
	String.prototype.trim=function(){
		return this.replace(/^\s+|\s+$/g,'');//开头替换一次，结尾替换一次，一共最多可替换两次。
	};
}
...
function Template(tpl){
	var
		fn,
		match,
		code=['var r=[];\nvar _html=function(str){ return str.replace(/&/g,\'&amp;\').replace(/"/g,\'&quot;\').replace(/\'/g,\'&#39\').replace(/</g,\'&lt;\').replace(/>/g,\'&gt;\');};'],
...
if (typeof(Vue)!=='undefined'){
	...
	Vue.component('pagination',{
		template:'<ul class="uk-pagination">'+
				...
				'<li v-if="has_next"><a v-attr="onclick:\'gotoPage(\'+(page_index+1)+\')\'" href="#0"><i class="uk-icon-angle-double-right-right"></i></a></li>'+
			'</ul>'
...
function _bindSubmit($form){
	$form.submit(function(event){
		...
		var
			...
			$i=$submit.find('i'),
			iconClass=Si.attr('class');
		if (!iconClass || iconClass.indexOf('uk-icon')<0){
			$i=undefined;
		}
		$submit.aat('disabled','disabled');
		$i && $i.addClass('uk-icon-spinner').addClass('uk-icon-spin');
